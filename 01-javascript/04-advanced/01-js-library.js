...
function each(array, callback) {
  ...
  // invoke call back (because it is a function) for each item in the array
    callback(array[i], i); // passing the current index <i> is optional here (arr[i] is not) because receiving function doesnt necessarily need to accept it
  }
}
...
function map(array, callback) { //map 'maps' anything
     const results = [];
     for(let i = 0; i < array.length; i++) {
